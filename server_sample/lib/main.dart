...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _initial() async {
    ...
    log(
      'WebSocket server is running on wss://ho-doan.com:${server.port}',
    );
  ...
  void _initialTCP() async {
    ...
    await for (var socket in server) {
      log('Connection from: ${socket.remoteAddress.address}:${socket.remotePort}');
      ...
      socket.listen(
        ...
        onDone: () {
          log('Client disconnected: ${socket.remoteAddress.address}:${socket.remotePort}');
          String? clientId;
          String? deviceId;
